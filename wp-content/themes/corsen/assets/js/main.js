(function ( $ ) {
	...
	$( window ).load(
		function () {
			setTimeout(
				function () {
					qodefSideByAttributesFilter.init();
				},
				8000
			);
		}
	...
	var qodefSwiper = {
		init: function ( settings ) {
			this.holder = $( '.qodef-swiper-container:not(.qodef-testimonials-list.qodef-hide-client-images--no.qodef-col-num--1)' );
	...
	var qodefSideCategoriesFilter = {
		init: function() {
			var countHolder      = $( '.wc-block-product-categories .wc-block-product-categories-list--depth-0, .woocommerce-widget-layered-nav-list' ),
				showMoreAppend   = '<a href="#" class="qodef-filter-show-more"><span class="qodef-filter-text">View More</span></a><a href="#" class="qodef-filter-show-more qodef-show-less"><span class="qodef-filter-text">View Less</span></a>',
			...
			qodefSideCategoriesFilter.qodefSideCategoriesFilterShowMore( countHolder );
		},
		qodefSideCategoriesFilterShowMore: function( $countHolder ) {
			...
			if ( countHolder.length ) {
				countHolder.each( function() {
					...
					if ( countHolderItems.length > 3 ) {
						var countHolderItemsHeight = countHolderItems.outerHeight(true),
							holderHeight           = countHolderItems.toArray().reduce( (curr, next, index) => {
								if ( index < 4 ) {
									return curr + $(next).outerHeight( true );
								}
								return curr;
							},20),
							holderHeightInit       = countHolderItemsHeight * countHolderItems.length + 60 + 'px';
						...
						countHolderItems.eq( 3 ).nextAll( itemClasses ).addClass( 'qodef--hidden' );
						...
						$thisList.find( '.qodef-filter-show-more' ).on(
							'click', function ( e ) {
								...
								if ( $btn.hasClass( 'qodef-show-less' ) ) {
									...
									countHolderItems.eq( 3 ).nextAll( itemClasses ).removeClass( 'qodef--hidden' );
									$thisList.css( "height", holderHeightInit );
								}
							}
						);
					}
	...
	qodef.qodefSideCategoriesFilter = qodefSideCategoriesFilter;

	var qodefSideByAttributesFilter = {
		init: function() {
			var countHolder      = $( '.wp-block-woocommerce-attribute-filter.qodef-view-more-button .wc-block-checkbox-list' ),
				showMoreAppend   = '<a href="#" class="qodef-filter-show-more"><span class="qodef-filter-text">View More</span></a><a href="#" class="qodef-filter-show-more qodef-show-less"><span class="qodef-filter-text">View Less</span></a>',
				itemClasses      = 'li',
				countHolderItems = countHolder.find( itemClasses );
			...
			qodefSideByAttributesFilter.qodefSideByAttributesFilterShowMore( countHolder );
		},
		qodefSideByAttributesFilterShowMore: function( $countHolder ) {
			// TODO: height calculation for hierarchical items
			var countHolder = $countHolder;
	...
	qodef.qodefSideByAttributesFilter = qodefSideByAttributesFilter;
			...
			$masonryItems.each(
				function () {
					...
					if ( ! isOneColumnLayout ) {
						...
						if ( $masonryItem.classList.contains( 'qodef-item--portrait_small' ) ) {
							elementHeight = Math.round( 1.371 * size + gap );
						}
						...
						if ( $masonryItem.classList.contains( 'qodef-item--tall-square' ) ) {
							elementWidth  = Math.round( 2 * size + gap );
							elementHeight = Math.round( 2.742 * size + gap );
...
(function ($) {
		...
		init: function () {
			...
			if ( firstLevelMenus.length ) {
				firstLevelMenus.each( function () {
					...
					mainMenu.append( '<span class="qodef-product-tabs-main-line"></span>' );
					...
					if ( menuItems.filter( '.active' ).length ) {
						...
						menuLine.css(
							'width',
							menuItems.filter( '.active' ).outerWidth() - parseInt(menuItems.filter( '.active' ).css('padding-right'), 10)
						);
					} else {
						initialOffset = menuItems.first().offset().left;
						menuLine.css(
							'width',
							menuItems.first().outerWidth() - parseInt(menuItems.first().css('padding-right'), 10)
						);
					}
					...
					menuItems.mouseenter( function () {
						var menuItem       = $( this ),
							menuItemWidth  = menuItem.outerWidth() - parseInt(menuItem.css('padding-right'), 10),
					...
					menuItems.mouseleave( function () {
						...
						if ( menuItem.hasClass( 'active' ) ) {
							...
							var activeLi    = menuItems.filter( '.active' ),
								activeWidth = activeLi.outerWidth() - parseInt(activeLi.css('padding-right'), 10),
	...
	qodef.qodefBottomLineFortabs = qodefBottomLineFortabs;
