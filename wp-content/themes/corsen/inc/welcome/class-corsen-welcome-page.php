...
if ( ! class_exists( 'Corsen_Welcome_Page' ) ) {
	/**
	 * Welcome page theme class
	 */
	class Corsen_Welcome_Page {
		private static $instance;

		/**
		 * @return Corsen_Welcome_Page
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_corsen_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_corsen_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_corsen_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'corsen_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'corsen_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( CORSEN_ROOT_DIR . '/screenshot.png' ) ? CORSEN_ROOT . '/screenshot.png' : CORSEN_ROOT . '/screenshot.jpg';

			corsen_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			...
			if ( 'appearance_page_corsen_welcome_page' === $hook ) {
				wp_enqueue_style( 'corsen-welcome-page-style', CORSEN_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}

	Corsen_Welcome_Page::get_instance();
}
