...
if ( ! function_exists( 'corsen_get_search_page_excerpt_length' ) ) {
	/**
	 * Function that return number of characters for excerpt on search page
	 *
	 * @return int
	 */
	function corsen_get_search_page_excerpt_length() {
		$length = apply_filters( 'corsen_filter_post_excerpt_length', 180 );

		return intval( $length );
	}
}

if ( ! function_exists( 'corsen_override_search_block_templates' ) ) {
	/**
	 * Function that override `core/search` block template
	 *
	 * @see register_block_core_search()
	 */
	function corsen_override_search_block_templates( $atts ) {
		if ( ! empty( $atts ) && isset( $atts['render_callback'] ) && 'render_block_core_search' === $atts['render_callback'] && function_exists( 'styles_for_block_core_search' ) ) {
			$atts['render_callback'] = 'corsen_render_block_core_search';
		}

		return $atts;
	}

	add_filter( 'block_type_metadata_settings', 'corsen_override_search_block_templates' );
}

if ( ! function_exists( 'corsen_render_block_core_search' ) ) {
	/**
	 ...
	 */
	function corsen_render_block_core_search( $attributes ) {
		static $instance_id = 0;

		$attributes = wp_parse_args(
			$attributes,
			array(
				'label'      => esc_html__( 'Search', 'corsen' ),
				'buttonText' => esc_html__( 'Search', 'corsen' ),
			)
		);
		...
		$label_markup = sprintf(
			...
			$input_id,
			empty( $attributes['label'] ) ? esc_html__( 'Search', 'corsen' ) : esc_html( $attributes['label'] )
		);
		...
		if ( $show_button ) {
			...
			if ( ! $use_icon_button ) {
				...
				$button_internal_markup = corsen_get_svg_icon( 'search' );
			}
