...
if ( ! function_exists( 'corsen_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function corsen_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'CorsenCore_Highlight_Shortcode';

		return $shortcodes;
	}

	add_filter( 'corsen_core_filter_register_shortcodes', 'corsen_core_add_highlight_shortcode' );
}

if ( class_exists( 'CorsenCore_Shortcode' ) ) {
	class CorsenCore_Highlight_Shortcode extends CorsenCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( CORSEN_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'corsen_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'corsen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'corsen-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'title'       => esc_html__( 'Highlight Text Position', 'corsen-core' ),
					'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'corsen-core' ),
					'group'       => esc_html__( 'Text Style', 'corsen-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Color', 'corsen-core' ),
					'group'      => esc_html__( 'Text Style', 'corsen-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Highlight Text Background Color', 'corsen-core' ),
					'group'      => esc_html__( 'Text Style', 'corsen-core' ),
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return corsen_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
