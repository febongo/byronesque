...
if ( ! function_exists( 'corsen_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function corsen_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'CorsenCore_Image_Marquee_Shortcode';

		return $shortcodes;
	}

	add_filter( 'corsen_core_filter_register_shortcodes', 'corsen_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'CorsenCore_Shortcode' ) ) {
	class CorsenCore_Image_Marquee_Shortcode extends CorsenCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'corsen_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'corsen_core_filter_image_marquee_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( CORSEN_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'corsen_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'corsen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'corsen-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					'field_type'    => 'text',
					'name'          => 'duration',
					'title'         => esc_html__( 'Animation Duration (Seconds)', 'corsen-core' ),
					'default_value' => '20',
				)
			);
			$this->set_option(
		...
		public function render( $options, $content = null ) {
			...
			return corsen_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
