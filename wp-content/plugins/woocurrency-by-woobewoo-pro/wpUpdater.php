	...
	class wpUpdaterWcu {
		...
		static public function getInstance($pluginDir, $pluginFile = '', $pluginSlug = '') {
			...
			if(!isset($instances[ $instKey ])) {
				$instances[ $instKey ] = new wpUpdaterWcu($pluginDir, $pluginFile, $pluginSlug);
			}
		...
		public function checkForPluginUpdate($checkedData) {
			...
			if ( isset( $checkedData->checked[ $this->_plugDir . '/' . $this->_plugFile ] ) ) {
				$request_args = array(
					'slug'    => $this->_plugSlug,
					'hash'    => constant( 'S_YOUR_SECRET_HASH_' . $this->_plugSlug ),
					'version' => $checkedData->checked[ $this->_plugDir . '/' . $this->_plugFile ],
				);
				if ( class_exists( 'frameWcu' ) && frameWcu::_()->getModule( 'license' ) && ! frameWcu::_()->getModule( 'license' )->getModel()->isExpired() ) {
					$license                 = frameWcu::_()->getModule( 'license' )->getModel()->getCredentials();
		...
		public function prepareRequest($action, $args) {
			...
			return array(
				...
				'user-agent' => $this->_getUserAgentHash(). '/' . $wp_version . '; ' . WCU_SITE_URL. ';'. $this->getIP()
			);	
		}
