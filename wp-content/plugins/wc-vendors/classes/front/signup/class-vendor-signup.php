...
class WCV_Vendor_Signup {
    ...
    public function vendor_registration_errors( $errors, $sanitized_user_login, $user_email ) {
        if ( ! isset( $_POST['apply_for_vendor'] ) || ! wp_verify_nonce( wsanitize_text_field( wp_unslash( $_POST['apply_for_vendor_nonce'] ) ), 'apply_for_vendor' ) ) {
            return;
        }
