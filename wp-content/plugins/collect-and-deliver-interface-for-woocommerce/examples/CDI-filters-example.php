...
//
  ...
  //****** Model filter to force a parcel CDI status (Blocked or Released) at the Gateway parcel introduction
  add_filter( 'cdi_filterstring_gateway_statusatinit','example_cdi_filterstring_gateway_statusatinit', 10, 3) ;
  function example_cdi_filterstring_gateway_statusatinit ($cdi_status, $orderid, $array_for_carrier) {
    // This example search for orders having for its products a special shipping class or having a special category
    $new_cdi_status = $cdi_status ;
    global $woocommerce ; 
    ...
    foreach( $items as $item ) { 
      ...
      $shipping_class_slug = get_term( $shipping_class_id, 'product_shipping_class' )->slug;
      $category_slugs = wp_get_post_terms( $item->get_product_id(), 'product_cat', ['fields' => 'slugs'] );
      if (($shipping_class_slug == "expedition-a-differer") or in_array('produit-en-precommande', $category_slugs)) {
        $new_cdi_status = 'close' ;
        break ;
      }
    }
    return $new_cdi_status ;	  
  }
