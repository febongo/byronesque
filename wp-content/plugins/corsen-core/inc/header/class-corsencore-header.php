...
abstract class CorsenCore_Header {
	...
	public function __construct() {
		...
		add_action( 'corsen_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		add_action( 'corsen_core_action_before_main_css', array( $this, 'enqueue_additional_assets' ) );
		add_filter( 'corsen_filter_localize_main_js', array( $this, 'set_global_javascript_variables' ) );
		add_filter( 'corsen_core_filter_content_margin', array( $this, 'get_content_margin' ) );
		add_filter( 'corsen_core_filter_title_padding', array( $this, 'get_title_padding' ) );
		add_filter( 'corsen_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'corsen_filter_header_inner_class', array( $this, 'set_header_inner_classes' ), 10, 2 );
		add_filter( 'corsen_core_filter_nav_menu_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'corsen_core_filter_nav_menu_narrow_header_selector', array( $this, 'set_nav_menu_narrow_header_selector' ) );
	}
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'corsen_core_filter_header_template', $parameters );

		return corsen_core_get_template_part( 'header/layouts/' . $this->get_layout(), 'templates/' . $this->get_layout(), $this->get_layout_slug(), $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
	...
	public function content_behind_header() {
		return 'yes' === corsen_core_get_post_value_through_levels( 'qodef_content_behind_header' );
	}
	...
	function set_header_height() {
		$header_height = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;

		$this->header_height = apply_filters( 'corsen_core_filter_set_header_height', $header_height );
	}

	function set_header_inner_classes( $class, $layout ) {
		// Check is content in grid
		$class[] = 'yes' === corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_in_grid' ) ? 'qodef-content-grid' : '';

		// Check header skin
		$header_skin = corsen_core_get_post_value_through_levels( 'qodef_header_skin' );
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_height' );
		$background_color = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_color' );
        $background_image = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_background_image' );
		...
		if ( ! empty( $styles ) ) {
			...
			if ( 'vertical-sliding' === $this->get_layout() ) {
                $style .= qode_framework_dynamic_style( '.qodef-header--' . $this->get_layout() . ' #qodef-page-header .qodef-vertical-sliding-area.qodef--static', $styles );
                $style .= qode_framework_dynamic_style( '.qodef-header--' . $this->get_layout() . ' #qodef-page-header .qodef-vertical-sliding-area.qodef--dynamic', $styles );
            }
		}

		$inner_styles = array();

		$side_padding = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_padding' );
        $side_margin  = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_side_margin' );
		$border_color = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_color' );
		$border_width = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_width' );
		$border_style = corsen_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_layout() . '_header_border_style' );
