...
if ( ! function_exists( 'corsen_core_add_product_list_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function corsen_core_add_product_list_shortcode( $shortcodes ) {
		$shortcodes[] = 'CorsenCore_Product_List_Shortcode';

		return $shortcodes;
	}

	add_filter( 'corsen_core_filter_register_shortcodes', 'corsen_core_add_product_list_shortcode' );
}

if ( class_exists( 'CorsenCore_List_Shortcode' ) ) {
	class CorsenCore_Product_List_Shortcode extends CorsenCore_List_Shortcode {
		...
		public function __construct() {
			...
			$this->set_layouts( apply_filters( 'corsen_core_filter_product_list_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'corsen_core_filter_product_list_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( CORSEN_CORE_PLUGINS_URL_PATH . '/woocommerce/shortcodes/product-list' );
			$this->set_base( 'corsen_core_product_list' );
			$this->set_name( esc_html__( 'Product List', 'corsen-core' ) );
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Enable Order By Filter', 'corsen-core' ),
					'options'       => corsen_core_get_select_type_options_pool( 'no_yes', false ),
					'default_value' => 'no',
					'group'         => esc_html__( 'Additional', 'corsen-core' ),
				)
			);
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Enable Category Filter', 'corsen-core' ),
					'options'       => corsen_core_get_select_type_options_pool( 'no_yes', false ),
					'default_value' => 'no',
					'group'         => esc_html__( 'Additional', 'corsen-core' ),
				)
			...
			$this->set_option(
				array(
					'field_type'  => 'text',
					'name'        => 'filter_tax__in',
					'title'       => esc_html__( 'Filter Taxonomy IDs', 'corsen-core' ),
					'description' => esc_html__( 'Separate filter taxonomy IDs with commas, and enable Product Category Taxonomy Slug in Query options.', 'corsen-core' ),
					'group'       => esc_html__( 'Additional', 'corsen-core' ),
					'dependency'  => array(
						'show' => array(
			...
			$this->set_option(
				array(
					...
					'dependency'    => array(
						'show' => array(
							'layout' => array(
								'values'        => array( 'info-on-image', 'info-boxed' ),
								'default_value' => '',
							),
						),
					),
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'simple_button',
					'title'      => esc_html__( 'Simple Button', 'corsen-core' ),
					'options'    => corsen_core_get_select_type_options_pool( 'no_yes', false ),
					'dependency' => array(
						'show' => array(
							'layout' => array(
								'values'        => array( 'info-boxed' ),
								'default_value' => '',
							),
						),
					),
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'corsen_core_product_list', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			$atts['product_prices']        = corsen_core_get_filtered_price();

			$atts['holder_classes'] = $this->get_holder_classes( $atts );
			$atts['query_result']   = new \WP_Query( corsen_core_get_query_params( $atts ) );
			$atts['slider_attr']    = $this->get_slider_data( $atts );
			$atts['data_attr']      = corsen_core_get_pagination_data( CORSEN_CORE_REL_PATH, 'plugins/woocommerce/shortcodes', 'product-list', 'product', $atts );

			$atts['this_shortcode'] = $this;

			return corsen_core_get_template_part( 'plugins/woocommerce/shortcodes/product-list', 'templates/content', $atts['behavior'], $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = 'yes' === $atts['simple_button'] ? 'qodef--simple-button' : '';
