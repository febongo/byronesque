...
if ( ! function_exists( 'corsen_core_add_image_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function corsen_core_add_image_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'CorsenCore_Image_With_Text_Shortcode';

		return $shortcodes;
	}

	add_filter( 'corsen_core_filter_register_shortcodes', 'corsen_core_add_image_with_text_shortcode' );
}

if ( class_exists( 'CorsenCore_Shortcode' ) ) {
	class CorsenCore_Image_With_Text_Shortcode extends CorsenCore_Shortcode {

		public function __construct() {
			$this->set_layouts( apply_filters( 'corsen_core_filter_image_with_text_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'corsen_core_filter_image_with_text_extra_options', array() ) );

			parent::__construct();
		}

		public function map_shortcode() {
			$this->set_shortcode_path( CORSEN_CORE_SHORTCODES_URL_PATH . '/image-with-text' );
			$this->set_base( 'corsen_core_image_with_text' );
			$this->set_name( esc_html__( 'Image With Text', 'corsen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image with text element', 'corsen-core' ) );
			$this->set_scripts(
				array(
			...
			$this->set_option(
				array(
					...
					'title'         => esc_html__( 'Show Image Border', 'corsen-core' ),
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'scrolling-image' => esc_html__( 'Scrolling Image', 'corsen-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'scrolling_frame_size',
					'title'      => esc_html__( 'Scrolling Frame Size', 'corsen-core' ),
					'options'    => array(
						'large' => esc_html__( 'Large', 'corsen-core' ),
						'small' => esc_html__( 'Small', 'corsen-core' ),
						'tall'  => esc_html__( 'Tall', 'corsen-core' ),
					),
					'dependency' => array(
						'show' => array(
							'image_action' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Scrolling Direction', 'corsen-core' ),
					'options'    => array(
						'vertical'   => esc_html__( 'Vertical', 'corsen-core' ),
						'horizontal' => esc_html__( 'Horizontal', 'corsen-core' ),
					),
					'dependency' => array(
						'show' => array(
							'image_action' => array(
			...
			$this->set_option(
				array(
					...
					'options'       => corsen_core_get_select_type_options_pool( 'title_tag', true, array(), array( 'span' => 'SPAN' ) ),
					'default_value' => 'h4',
			...
			$this->set_option(
				array(
					'field_type' => 'select',
					'name'       => 'enable_hover',
					'title'      => esc_html__( 'Enable Hover', 'corsen-core' ),
					'options'    => corsen_core_get_select_type_options_pool( 'yes_no', false ),
				)
			);
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'corsen_core_image_with_text', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return corsen_core_get_template_part( 'shortcodes/image-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty( $atts['show_image_border'] ) ? 'qodef-show-image-border--' . $atts['show_image_border'] : '';
