...
if ( ! function_exists( 'corsen_core_add_icon_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function corsen_core_add_icon_shortcode( $shortcodes ) {
		$shortcodes[] = 'CorsenCore_Icon_Shortcode';

		return $shortcodes;
	}

	add_filter( 'corsen_core_filter_register_shortcodes', 'corsen_core_add_icon_shortcode', 8 );
}

if ( class_exists( 'CorsenCore_Shortcode' ) ) {
	class CorsenCore_Icon_Shortcode extends CorsenCore_Shortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( CORSEN_CORE_SHORTCODES_URL_PATH . '/icon' );
			$this->set_base( 'corsen_core_icon' );
			$this->set_name( esc_html__( 'Icon', 'corsen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon element', 'corsen-core' ) );
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_class',
			...
			$this->set_option(
				array(
					...
					'options'    => array(
						...
						'tiny'    => esc_html__( 'Tiny', 'corsen-core' ),
						'small'   => esc_html__( 'Small', 'corsen-core' ),
						'medium'  => esc_html__( 'Medium', 'corsen-core' ),
						'large'   => esc_html__( 'Large', 'corsen-core' ),
						'huge'    => esc_html__( 'Huge', 'corsen-core' ),
					),
				)
			);
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'custom_size',
					'title'      => esc_html__( 'Custom Size', 'corsen-core' ),
				)
			);
			$this->set_option(
				array(
			...
			$this->set_option(
				array(
					'field_type' => 'text',
					'name'       => 'shape_size',
					'title'      => esc_html__( 'Shape Size', 'corsen-core' ),
					'group'      => esc_html__( 'Style', 'corsen-core' ),
					'dependency' => array(
						'show' => array(
							'icon_layout' => array(
			...
			$this->set_option(
				array(
					...
					'title'      => esc_html__( 'Vertical Offset', 'corsen-core' ),
					'group'      => esc_html__( 'Style', 'corsen-core' ),
				)
			);
			$this->set_option(
		...
		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'corsen_core_icon', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return corsen_core_get_template_part( 'shortcodes/icon', 'templates/icon', '', $atts );
		}
