...
if ( ! function_exists( 'corsen_core_add_team_archive_sidebar_options' ) ) {
	/**
	 * Function that add sidebar options for team archive module
	 */
	function corsen_core_add_team_archive_sidebar_options( $tab ) {
		...
		if ( $tab ) {
			$tab->add_field_element(
				array(
					...
					'description'   => esc_html__( 'Choose default sidebar layout for team archives', 'corsen-core' ),
			...
			if ( ! empty( $custom_sidebars ) && count( $custom_sidebars ) > 1 ) {
				$tab->add_field_element(
					array(
						...
						'description' => esc_html__( 'Choose a custom sidebar to display on team archives', 'corsen-core' ),
	...
	add_action( 'corsen_core_action_after_team_options_archive', 'corsen_core_add_team_archive_sidebar_options' );
}

if ( ! function_exists( 'corsen_core_add_team_single_sidebar_options' ) ) {
	/**
	 * Function that add sidebar options for team single module
	 */
	function corsen_core_add_team_single_sidebar_options( $tab ) {
	...
	add_action( 'corsen_core_action_after_team_options_single', 'corsen_core_add_team_single_sidebar_options' );
}
