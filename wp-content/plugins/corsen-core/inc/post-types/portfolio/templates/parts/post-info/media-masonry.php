...
$masonry_classes   = '';
$number_of_columns = corsen_core_get_post_value_through_levels( 'qodef_portfolio_columns_number' );
$masonry_classes  .= ! empty( $number_of_columns ) ? ' qodef-col-num--' . $number_of_columns : ' qodef-col-num--3';

$space_between_items = corsen_core_get_post_value_through_levels( 'qodef_portfolio_space_between_items' );
...
if ( ! empty( $portfolio_media ) ) { ?>
	<div class="qodef-e qodef-grid qodef-layout--masonry qodef-items--fixed qodef-responsive--predefined <?php echo esc_attr( $masonry_classes ); ?>">
		<div class="qodef-grid-inner qodef-magnific-popup qodef-popup-gallery">
			...
			corsen_core_theme_template_part( 'masonry', 'templates/sizer-gutter', '', 'masonry' );
			...
			foreach ( $portfolio_media as $media ) {
				...
				corsen_core_template_part( 'post-types/portfolio', 'templates/parts/media/media', $type . '-masonry', $params );
			}
			?>
		</div>
	</div>
